...
int main(int argc, char *argv[]) {
  ...
  bool first_start = SETTINGS.Load("assets/settings.bin");

  LOCAL.LoadLocalsFromFolder(local_folder);
  sf::RenderWindow window;
  Renderer rend("shaders/compute/MAIN.cfg");
  ...
  SetPointers(&window, &scene, &overlays, &rend, &renderTexture, &screenshotTexture, &main_txt, &screenshot_txt, &rect, &rect_scrshot, &shader);
